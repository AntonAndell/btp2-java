...
public class ProtocolPrefixNetworkAddress {
    ...
    public ProtocolPrefixNetworkAddress(List<String> protocols, String net, String account) {
        this.protocols = protocolsString(protocols);
        this.net = net;
        this.account = account;
    }
    ...
    public ProtocolPrefixNetworkAddress(List<String> protocols, NetworkAddress address) {
        this.protocols = protocolsString(protocols);
        this.net = address.net;
    ...
    public List<String> protocolsList() {
        return parseProtocols(protocols);
    }

    public static List<String> parseProtocols(String protocols) {
        List<String> protocolList = new ArrayList<>();
        String remaining = protocols;

        int protocolIdx = remaining.indexOf(DELIM_PROTOCOLS);
        while (protocolIdx >= 0) {
            protocolList.add(remaining.substring(0, protocolIdx));
            remaining = remaining.substring(protocolIdx + DELIM_PROTOCOL.length());
            protocolIdx = remaining.indexOf(DELIM_PROTOCOLS);
        }
    ...
    private static String protocolsString(List<String> protocols) {
        String result = "";
        int i;
        for (i = 0; i < protocols.size() - 1; i++) {
            result = result + protocols.get(i) + "|";
        }
        result = result + protocols.get(++i);
        return result;
    }
